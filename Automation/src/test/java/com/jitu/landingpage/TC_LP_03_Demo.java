...
public class TC_LP_03_Demo extends BaseTest {
    ...
    /**
     * Shared helper — scrolls the terminal into view and waits until the body
     * has a non-zero rendered height (guards against CSS animation
     * false-negatives).
     */
    private WebElement scrollAndAwaitTerminalBody() {
    ...
    void LP_020_DemoPanelShowsSampleOutput() {
        // FIX #4: body is awaited first via the helper; terminal is re-located
        // afterwards so it is never stale when terminalTitle is queried
        WebElement body = scrollAndAwaitTerminalBody();
        WebElement terminal = driver.findElement(By.cssSelector("div[class*='_terminal']"));

        // --- Terminal title ---
        WebElement terminalTitle = wait.until(d -> {
            WebElement el = terminal.findElement(By.cssSelector("span[class*='_terminalTitle']"));
            String text = el.getText();
            return (text != null && !text.isEmpty()) ? el : null;
        });
        ...
        // FIX #3: progress hint is conditional per spec ("if present"),
        // so guard with findElements to avoid NoSuchElementException
        List<WebElement> cursors = body.findElements(By.cssSelector("div[class*='_typingCursor']"));
        if (!cursors.isEmpty() && cursors.get(0).isDisplayed()) {
            assertTrue(cursors.get(0).getText().contains("generating"),
                    "Progress hint found but text did not contain 'generating'");
        }
    }
